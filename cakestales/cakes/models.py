...
class Wishlist(BaseClass):
    ...
    def __str__(self):
        ...
        return f'{self.user.username} whishlist'
    
    class Meta:

        verbose_name = 'Whishlists'
...
class DeliveryAddress(BaseClass):
    ...
    user = models.ForeignKey('authentication.Profile',on_delete=models.CASCADE) # related_name can be used in foreign key to set a name for that model and can use that name for calling on reverse calling, HERE after cascade use related_name = 'deliveryaddress'
